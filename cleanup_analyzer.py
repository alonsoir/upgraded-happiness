...
def analyze_fix_scripts(fix_scripts):
    ...
    for file_path, size in fix_scripts:
        ...
        try:
            ...
            print(
                f"   📊 {analysis['lines']} líneas, {analysis['functions']} funciones, {analysis['classes']} clases"
            )
...
def generate_cleanup_strategy(categories, script_analysis):
    ...
    duplicate_size = sum(
        sum(size for _, size in group[1:]) for group in categories["duplicates"]
    )
    ...
    print(f"\n💾 ESPACIO A LIBERAR:")
    print(
        f"   📦 Backups: {backup_size / 1024:.1f}KB ({len(categories['backup_files'])} archivos)"
    )
    print(
        f"   🗑️  Temporales: {temp_size / 1024:.1f}KB ({len(categories['temp_files'])} archivos)"
    )
    print(
        f"   👥 Duplicados: {duplicate_size / 1024:.1f}KB ({len(categories['duplicates'])} grupos)"
    )
    ...
    print(
        f"   📜 {len(categories['fix_scripts'])} scripts ({fix_total_size / 1024:.1f}KB total)"
    )
    ...
    for file_path, size in categories["fix_scripts"]:
        ...
        print(
            f"   • {file_path.name} - {', '.join(keywords) if keywords else 'general'}"
        )
    ...
    action_plan = {
        "immediate_delete": {
            ...
            "duplicates": [
                [str(f) for f, _ in group[1:]] for group in categories["duplicates"]
            ],
        },
        "consolidate": {"fix_scripts": [str(f) for f, _ in categories["fix_scripts"]]},
        "preserve": {
