...
def backup_and_clean():
    ...
    if os.path.exists(pb2_file):
        ...
        print(f"✅ Backup del archivo corrompido: {backup_file}")

        # Eliminar archivo corrompido
        os.remove(pb2_file)
        print(f"✅ Eliminado archivo corrompido: {pb2_file}")


def regenerate_protobuf():
    """Regenerar protobuf usando protoc"""

    print("🔄 Regenerando protobuf...")
    ...
    try:
        ...
        if os.path.exists(pb2_file):
            ...
            print(f"❌ No se creó el archivo: {pb2_file}")
            return False

    except subprocess.CalledProcessError as e:
        print(f"❌ Error ejecutando protoc: {e}")
...
def create_compatible_version():
    """Crear una versión compatible manualmente si protoc falla"""
...
def test_import():
    """Probar que la importación funcione"""
...
"""
    ...
    try:
        ...
        success = "✅ Serialización OK" in result.stdout
        return success
    finally:
...
if __name__ == "__main__":
    print("🚀 REGENERACIÓN LIMPIA DE PROTOBUF")
    print("=" * 50)
    ...
    if regenerate_protobuf():
        ...
        if test_import():
            print("\\n🎉 ¡PROTOBUF REGENERADO EXITOSAMENTE!")
            print("\\n🧪 Ahora ejecuta:")
            print("   python test_protobuf_simple.py")
            print("   python agent_scapy_fixed.py")
        else:
            print("\\n⚠️  Protobuf regenerado pero hay problemas de importación")
            print("Creando versión compatible...")
            create_compatible_version()
            if test_import():
                print("✅ Versión compatible funciona")
    else:
        print("\\n⚠️  Error con protoc, creando versión compatible...")
        create_compatible_version()
        if test_import():
            print("✅ Versión compatible funciona")
