...
class PipelineAnalyzer:
    ...
    def generate_connectivity_matrix(self, endpoints_map):
        ...
        for comp in components:
            if comp == 'firewall_rules':  # Skip firewall_rules from matrix
                continue
            print(f"{comp[:8]:>9}", end="")
        print()

        for source in components:
            if source == 'firewall_rules':  # Skip firewall_rules from matrix
                continue

            print(f"{source[:8]:>8} ", end="")

            for target in components:
                if target == 'firewall_rules':  # Skip firewall_rules from matrix
                    continue
    ...
    def run_comprehensive_analysis(self):
        ...
        # 6. Analizar fleet específicamente
        ...
        all_issues = connectivity_issues + fleet_issues + role_issues
        ...
        print(f"\n🔧 RECOMENDACIONES:")
        print(f"   1. Dashboard debería ser SOLO monitor - remover firewall_fleet config")
        print(f"   2. ML_Detector debe tener DUAL output: Dashboard (5580) + Scheduler")
        print(f"   3. Scheduler_Firewall debe manejar TODO el fleet management")
        print(f"   4. Verificar que todos los componentes tengan ETCD habilitado")
        print(f"   5. Confirmar que endpoints coincidan: ML->Dashboard y ML->Scheduler")
        print(f"   6. Firewall_Agent debe comunicarse SOLO con Scheduler, no Dashboard")
