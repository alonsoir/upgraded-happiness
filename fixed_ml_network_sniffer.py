#!/usr/bin/env python3
"""
Fixed ML Network Sniffer
Corrige la detección de modelos vs scalers
"""
...
class FixedMLSniffer:
    ...
    def _detect_and_load_models(self):
        ...
        # Mostrar scalers encontrados (para info)
        scalers_found = glob.glob("models/*scaler*.joblib")
        if scalers_found:
            print(f"\n📋 Scalers encontrados (para referencia):")
            for scaler in scalers_found:
                print(f"   - {scaler}")

        if not models_found:
            print("❌ No se encontraron modelos válidos!")
            print("\n🔧 ARCHIVOS DISPONIBLES:")
            all_joblib = glob.glob("models/*.joblib")
            for file in sorted(all_joblib):
                if "scaler" in file:
                    print(f"   📏 {file} (scaler)")
                else:
                    print(f"   🤖 {file} (posible modelo)")
            sys.exit(1)
...
def main():
    """Función principal"""
    print("🎯 Fixed ML Network Sniffer v3.0")
    print("=================================")

    sniffer = FixedMLSniffer(
