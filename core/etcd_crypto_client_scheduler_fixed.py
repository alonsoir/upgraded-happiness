#!/usr/bin/env python3
"""
core/etcd_crypto_client_scheduler_fixed.py
...
class ETCDCryptoClientSchedulerFirewall:
    ...
    def __init__(self, scheduler_config_path: str, rules_config_path: str = None):
        """
        Inicializar cliente ETCD REAL

        Args:
            scheduler_config_path: Ruta al config JSON del componente
    ...
    def _load_component_config(self):
        ...
        try:
            ...
            self.logger.info(f"✅ {'scheduler_firewall'.title()} config loaded successfully")

        except json.JSONDecodeError as e:
            raise ValueError(f"❌ Invalid JSON in {'scheduler_firewall'} config: {e}")
        except Exception as e:
            raise RuntimeError(f"❌ Failed to load {'scheduler_firewall'} config: {e}")
    ...
    def _extract_etcd_config(self):
        ...
        if 'etcd_crypto' not in self.component_config:
            raise KeyError(
                f"❌ REQUIRED section 'etcd_crypto' not found in {'scheduler_firewall'} config.\n"
    ...
    async def _register_configs_in_etcd(self):
        ...
        try:
            ...
            component_config_key = "/upgraded-happiness/v31/components/scheduler/config"
            component_config_data = {
                "config_type": "scheduler_config",
            ...
            self.logger.info(f"✅ Scheduler_Firewall config registered at: {component_config_key}")

            # Registrar reglas firewall (si aplica)
            if self.rules_config:
                rules_config_key = "/upgraded-happiness/v31/components/scheduler/firewall_rules"
                rules_config_data = {
                    "config_type": "firewall_rules",
                    "node_id": self.etcd_config.node_id,
                    "managed_by": "scheduler_firewall",
                    "config_data": self.rules_config,
                    "timestamp": datetime.now().isoformat(),
                    "rules_summary": "Firewall rules for scheduler_firewall"
                }
            ...
            self.logger.info(f"📊 Scheduler_Firewall config registration completed successfully")
    ...
    async def initialize_crypto(self) -> bool:
        ...
        try:
            ...
            self.crypto_token = {
                'key': f"{'scheduler_firewall'}_key_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                'version': 'v3.1.0',
                'timestamp': datetime.now().isoformat()
            }
...
async def setup_scheduler_firewall_crypto(scheduler_config_path: str, rules_config_path: str = None) -> bool:
    """
    ...
    """
    global _etcd_client
    _etcd_client = ETCDCryptoClientSchedulerFirewall(scheduler_config_path, rules_config_path)
...
def get_scheduler_crypto_status() -> Dict:
    """Obtener status crypto para scheduler_firewall"""
    global _etcd_client
    if not _etcd_client:
...
if __name__ == "__main__":
    ...
    async def test_scheduler_etcd():
        ...
        # python core/etcd_crypto_client_scheduler_fixed.py

        print("🚨 Para test real, usa un config válido y ETCD corriendo")

    if len(sys.argv) > 1:
        print(f"🔧 Para integrar: from core.etcd_crypto_client_scheduler_fixed import setup_scheduler_firewall_crypto, get_scheduler_firewall_pipeline_key")
    else:
        asyncio.run(test_scheduler_etcd())
