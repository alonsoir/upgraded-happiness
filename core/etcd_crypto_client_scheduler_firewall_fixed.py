...
🔥 Cliente ETCD REAL - SCHEDULER_FIREWALL
================================================================================
BASADO EN LA PLANTILLA DEL GEOIP - SIN FALLBACKS, SOLO ETCD PURO
- Recibe DOS ficheros JSON (scheduler config + firewall rules)
- Los guarda en ETCD
...
class SchedulerFirewallETCDConfig:
    """Configuración ETCD extraída desde el config JSON del scheduler"""
    etcd_host: str
    etcd_port: int
    cluster_name: str
    node_id: str
...
class ETCDCryptoClientSchedulerFirewall:
    """🔥 Cliente ETCD REAL para SCHEDULER_FIREWALL"""

    def __init__(self, scheduler_config_path: str, firewall_rules_path: str):
        """
        Inicializar cliente ETCD REAL para scheduler_firewall

        Args:
            scheduler_config_path: Ruta al config JSON del scheduler
        ...
        # Setup logging
        self.logger = logging.getLogger(f"scheduler_firewall_etcd_client")
        if not self.logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                '%(asctime)s - SCHEDULER_FIREWALL_ETCD - %(levelname)s - %(message)s'
            )
    ...
    def _load_configs(self):
        ...
        try:
            # 1. Cargar scheduler config
            with open(self.scheduler_config_path, 'r') as f:
                self.scheduler_config = json.load(f)
            self.logger.info(f"✅ Scheduler config loaded: {self.scheduler_config_path}")

            # 2. Cargar firewall rules
            ...
            raise ValueError(f"❌ Invalid JSON in scheduler_firewall configs: {e}")
        except Exception as e:
            raise RuntimeError(f"❌ Failed to load scheduler_firewall configs: {e}")

    def _extract_etcd_config(self):
        """Extraer configuración ETCD del config del scheduler"""
        if not self.scheduler_config:
            raise ValueError("❌ Scheduler config must be loaded first")

        if 'etcd_crypto' not in self.scheduler_config:
            raise KeyError(
                f"❌ REQUIRED section 'etcd_crypto' not found in scheduler config.\n"
                f"   Add 'etcd_crypto' section to {self.scheduler_config_path}"
            )
        ...
        if missing_fields:
            raise KeyError(
                ...
                f"   Add these fields to {self.scheduler_config_path}"
            )
    ...
    async def _register_configs_in_etcd(self):
        """🎯 Registrar AMBAS configuraciones en ETCD - PATRÓN SCHEDULER_FIREWALL"""
        try:
            self.logger.info(f"📊 Registering scheduler_firewall configs in ETCD...")
            ...
            # 1. Registrar SCHEDULER CONFIG
            scheduler_config_key = "/upgraded-happiness/v31/components/scheduler_firewall/config"
            scheduler_config_data = {
                "config_type": "scheduler_firewall_config",
                "node_id": self.etcd_config.node_id,
                "component_type": "scheduler_firewall",
                "config_data": self.scheduler_config,
                "source_file": self.scheduler_config_path,
                "timestamp": datetime.now().isoformat(),
                "config_version": "3.1.0"
            }

            etcd_client.put(scheduler_config_key, json.dumps(scheduler_config_data, indent=2))
            self.logger.info(f"✅ Scheduler config registered at: {scheduler_config_key}")

            # 2. Registrar FIREWALL RULES
            firewall_rules_key = "/upgraded-happiness/v31/components/scheduler_firewall/rules"
            firewall_rules_data = {
            ...
            self.logger.info(f"📊 Scheduler_Firewall dual config registration completed successfully")

        except Exception as e:
            self.logger.error(f"❌ Config registration failed: {e}")
            # 🚨 HARD FAIL - NO FALLBACKS
            raise RuntimeError(f"ETCD config registration failed for scheduler_firewall: {e}")

    def _generate_rotative_token(self):
        """Generar token rotativo para scheduler_firewall"""
        try:
            timestamp = int(time.time())
            token_data = {
                'component': 'scheduler_firewall',
                'node_id': self.etcd_config.node_id,
                'token_id': str(uuid.uuid4()),
                'created_at': timestamp,
                'expires_at': timestamp + 3600,  # 1 hora
                'version': 'v3.1.0',
                'permissions': ['scheduler', 'firewall', 'read', 'write']
            }
            ...
            self.crypto_token = {
                'key': f"scheduler_firewall_key_{token_hash[:16]}",
    ...
    async def initialize_crypto(self) -> bool:
        ...
        """
        try:
            self.logger.info(f"🚀 Initializing scheduler_firewall ETCD crypto...")
            ...
            self.logger.info(f"✅ Scheduler_Firewall crypto initialization successful!")
            self.logger.info(f"🔑 Token version: {self.crypto_token['version']}")
            self.logger.info(f"📁 Configs stored: scheduler + firewall rules")
            ...
            raise RuntimeError(f"ETCD initialization failed for scheduler_firewall: {e}")

    def get_pipeline_key(self) -> Optional[str]:
        """Obtener UPGRADED_HAPPINESS_PIPELINE_KEY para scheduler_firewall"""
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.logger.info("🧹 Scheduler_Firewall ETCD client cleaned up")
        except Exception as e:
            self.logger.error(f"❌ Error in cleanup: {e}")


# 🎯 FUNCIONES PÚBLICAS PARA COMPATIBILIDAD
async def setup_scheduler_firewall_crypto(scheduler_config_path: str, firewall_rules_path: str) -> bool:
    """
    Setup crypto para scheduler_firewall
    ...
    """
    global _etcd_client
    _etcd_client = ETCDCryptoClientSchedulerFirewall(scheduler_config_path, firewall_rules_path)

    success = await _etcd_client.initialize_crypto()
    if not success:
        raise RuntimeError(f"Failed to initialize scheduler_firewall ETCD crypto")

    return True


def get_scheduler_firewall_pipeline_key() -> Optional[str]:
    """Obtener pipeline key para scheduler_firewall"""
    global _etcd_client
    if not _etcd_client:
        return None
...
def get_scheduler_firewall_token() -> Optional[str]:
    """Obtener token rotativo actual para scheduler_firewall"""
    global _etcd_client
    if not _etcd_client:
        return None
...
def get_scheduler_firewall_crypto_status() -> Dict:
    """Obtener status crypto para scheduler_firewall - FUNCIÓN REQUERIDA"""
    global _etcd_client
    if not _etcd_client:
...
def cleanup_scheduler_firewall_crypto():
    """Limpiar recursos del cliente scheduler_firewall"""
    global _etcd_client
    if _etcd_client:
...
# Cliente global
_etcd_client: Optional[ETCDCryptoClientSchedulerFirewall] = None
...
if __name__ == "__main__":
    ...
    async def test_scheduler_firewall_etcd():
        """Test básico del cliente ETCD"""
        print(f"🧪 Testing scheduler_firewall ETCD client...")
        ...
        try:
            ...
            success = await setup_scheduler_firewall_crypto(scheduler_config_path, firewall_rules_path)
            ...
            if success:
                ...
                try:
                    while True:
                        ...
                        new_status = get_scheduler_firewall_crypto_status()
    ...
    if len(sys.argv) > 1:
        asyncio.run(test_scheduler_firewall_etcd())
    else:
        print(
            f"🔧 Para integrar: from core.etcd_crypto_client_scheduler_firewall_fixed import setup_scheduler_firewall_crypto, get_scheduler_firewall_pipeline_key, get_scheduler_firewall_crypto_status")
        print(
            f"📋 Usage: python core/etcd_crypto_client_scheduler_firewall_fixed.py <scheduler_config.json> <firewall_rules.json>")