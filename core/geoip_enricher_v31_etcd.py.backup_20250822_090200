#!/usr/bin/env python3
"""
geoip_enricher_v31_etcd.py - Enriquecedor GeoIP v3.1.0 TRIPARTITO CLEAN + ETCD CRYPTO
🌍 Enhanced GeoIP Enricher para Upgraded-Happiness (VERTICAL SCALING v3.1.0) + ETCD Integration
...
🔐 NUEVO: Integración ETCD crypto para pipeline distribuido
"""
...
# 🔐 NUEVO: Importar cliente ETCD crypto en lugar de CryptoZMQV31
from etcd_crypto_client_geoip_fixed import (
    setup_geoip_crypto,
...
class DistributedGeoIPEnricherVerticalV31ETCD:
    """
    GeoIP Enricher distribuido v3.1.0 TRIPARTITO CLEAN + ETCD CRYPTO
    🎯 NUEVO: Enriquecimiento TRIPARTITO (sniffer_node_geo + source_ip_geo + destination_ip_geo)
    ...
    def __init__(self, config_file: str, pipeline_key: Optional[str] = None):
        ...
        self.stats = {
            ...
            # 🔐 NUEVO: Estadísticas ETCD crypto
            'etcd_crypto_operations': 0,
            'pipeline_key_usage': 0,
            'crypto_errors': 0,
            'start_time': time.time(),
            'last_stats_time': time.time()
        }

        # 🔌 Setup ZeroMQ con optimizaciones verticales + ETCD crypto (MODIFICADO)
        self.context = zmq.Context()
        self.input_socket = None
        self.output_socket = None
        self.setup_sockets_vertical_etcd()
        ...
        self.logger.info(f"🌍 Distributed GeoIP Enricher VERTICAL v3.1.0 TRIPARTITO CLEAN + ETCD inicializado")
        ...
        self.logger.info(f"   🔐 ETCD crypto: {'✅' if pipeline_key else '❌'}")
    ...
    def _load_config_strict(self, config_file: str) -> Dict[str, Any]:
        ...
        required_fields = [
            ...
            "geoip", "logging", "monitoring", "distributed", "etcd_crypto"  # 🔐 NUEVO
        ]
    ...
    def setup_logging(self):
        ...
        if file_config.get("enabled", False):
            log_file = file_config.get("path", "logs/geoip_enricher_v31_etcd.log")
    ...
    def _log_v31_configuration(self):
        ...
        self.logger.info("🎯 Configuración de enriquecimiento v3.1.0 TRIPARTITO + ETCD:")
    ...
    def setup_sockets_vertical_etcd(self):
        """🔐 NUEVO: Configuración ZMQ + ETCD crypto - MODIFICADO del v3.0.0"""
        ...
        try:
            ...
            # 🔐 NUEVO: Configurar crypto usando ETCD pipeline key
            if self.config.get("crypto", {}).get("enabled", False):
                if not self.pipeline_key:
                    raise RuntimeError("❌ Crypto enabled but no ETCD pipeline key available")

                try:
                    # 🔐 NUEVO: Usar crypto simple con pipeline key en lugar de CryptoZMQV31
                    # Por ahora, simplemente loggear que crypto está habilitado
                    # TODO: Implementar crypto wrapper simple usando pipeline_key
                    self.logger.info("🔐 ETCD crypto habilitado para GeoIP Enricher v3.1.0")
                ...
                self.logger.info("🔐 Crypto deshabilitado")

            self.logger.info(f"🔌 Sockets ZMQ VERTICAL v3.1.0 + ETCD configurados:")
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    request.add_header('User-Agent', 'GeoIP-Enricher-v3.1.0-ETCD')
    ...
    def enrich_protobuf_event_v31_tripartite(self, protobuf_data: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            event.geo_enrichment.enricher_version = "3.1.0_tripartite_clean_etcd"
    ...
    def receive_protobuf_events_vertical(self):
        ...
        self.logger.info("📡 Iniciando thread de recepción protobuf VERTICAL v3.1.0 + ETCD...")
    ...
    def process_protobuf_events_vertical(self):
        ...
        self.logger.info("⚙️ Iniciando thread de procesamiento VERTICAL v3.1.0 TRIPARTITO + ETCD...")
    ...
    def send_enriched_events(self):
        ...
        self.logger.info("📤 Iniciando thread de envío vertical v3.1.0 + ETCD...")
        queue_timeout = self.config["processing"]["queue_timeout_seconds"]
    ...
    def monitor_performance_vertical(self):
        ...
        while self.running:
            ...
            self.vertical_manager.update_vertical_metrics()
            self._log_performance_stats_v31_tripartite_etcd()

    def _log_performance_stats_v31_tripartite_etcd(self):
        """Log de estadísticas v3.1.0 TRIPARTITO + ETCD"""
        now = time.time()
        ...
        self.logger.info(f"📊 GeoIP Enricher VERTICAL v3.1.0 TRIPARTITO + ETCD Stats:")
        ...
        self.logger.info(f"   🔐 ETCD crypto ops: {self.stats['etcd_crypto_operations']}")
        self.logger.info(f"   🔑 Pipeline key usage: {self.stats['pipeline_key_usage']}")
        ...
        for key in ['received', 'enriched', 'sent', 'failed_lookups', 'cache_hits', 'cache_misses',
                    ...
                    'etcd_crypto_operations', 'pipeline_key_usage']:
            self.stats[key] = 0
    ...
    def run(self):
        """Ejecutar el enriquecedor v3.1.0 TRIPARTITO + ETCD"""
        self.logger.info("🚀 Iniciando Distributed GeoIP Enricher VERTICAL v3.1.0 TRIPARTITO CLEAN + ETCD...")
        ...
        self.logger.info(f"✅ GeoIP Enricher VERTICAL v3.1.0 TRIPARTITO + ETCD iniciado con {len(threads)} threads")
        self.logger.info(f"   🎯 Enriquecimiento: TRIPARTITO (sniffer + source + destination)")
        self.logger.info(f"   🏠 Sniffer geo: calculado UNA SOLA VEZ")
        self.logger.info(f"   📦 Protobuf: {PROTOBUF_VERSION}")
        self.logger.info(f"   🌍 MaxMind: {'✅' if MAXMIND_AVAILABLE else '❌'}")
        ...
        try:
            ...
            self.logger.info("🛑 Deteniendo GeoIP Enricher v3.1.0 TRIPARTITO + ETCD...")
    ...
    def shutdown(self, threads):
        ...
        total_crypto_ops = self.stats.get('etcd_crypto_operations', 0)

        self.logger.info(f"📊 Stats finales v3.1.0 TRIPARTITO + ETCD - Runtime: {runtime:.1f}s")
        ...
        self.logger.info(f"   🔐 Total operaciones ETCD crypto: {total_crypto_ops}")
        ...
        self.logger.info("✅ Distributed GeoIP Enricher v3.1.0 TRIPARTITO + ETCD cerrado correctamente")


# 🚀 Main con integración ETCD
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print("   export UPGRADED_HAPPINESS_DEV_MODE=true  # Para modo desarrollo")
        print("   # Si no se configura IPAPI_TOKEN, se usará plan gratuito (1000 requests/mes)")
        print()
        print("🔐 ETCD crypto:")
    ...
    async def main():
        """Main async function con integración ETCD"""
        print("🔍 Iniciando GeoIP Enricher v3.1.0 TRIPARTITO + ETCD...")
        ...
        try:
            # 🔐 NUEVO: Setup ETCD crypto
            print("🔐 Configurando ETCD crypto para GeoIP Enricher...")
            if not await setup_geoip_crypto(config_file, testing_mode=dev_mode):
            ...
            # 🌍 Crear y ejecutar enricher
            enricher = DistributedGeoIPEnricherVerticalV31ETCD(config_file, pipeline_key)
            enricher.run()
