...
class SchedulerLogger:
    ...
    def __init__(self, node_id: str, log_config: dict):
        self.logger = logging.getLogger(f"scheduler_firewall_{node_id}")
        self.node_id = node_id

        # Configurar logging según JSON
        log_level = getattr(logging, log_config.get('level', 'INFO').upper())
        log_format = log_config.get('format',
                                    '%(asctime)s - %(name)s - %(levelname)s - [scheduler:{node_id}] [pid:{pid}] - %(message)s'
                                    )
        ...
        if console_config.get('enabled', True):
            ...
            print(f"✅ Scheduler console logging enabled: {console_config.get('level', 'INFO')}")

        # Handler de archivo
        file_config = log_config.get('handlers', {}).get('file', {})
        if file_config.get('enabled', True):
            file_path = file_config.get('path', 'logs/scheduler_firewall.log')
            ...
            try:
                ...
                print(f"✅ Scheduler file logging enabled: {file_path} ({file_config.get('level', 'INFO')})")

                # Log inicial
                self.logger.info(f"🚀 Scheduler Logger started - Node: {node_id} - PID: {os.getpid()}")

            except Exception as e:
                print(f"⚠️ Error configuring scheduler file logging: {e}")
        ...
        self.info("✅ Scheduler Logger configured successfully")

    def info(self, msg, *args, **kwargs):
        self.logger.info(f"[scheduler:{self.node_id}] [pid:{os.getpid()}] - {msg}", *args, **kwargs)

    def warning(self, msg, *args, **kwargs):
        self.logger.warning(f"[scheduler:{self.node_id}] [pid:{os.getpid()}] - {msg}", *args, **kwargs)

    def error(self, msg, *args, **kwargs):
        self.logger.error(f"[scheduler:{self.node_id}] [pid:{os.getpid()}] - {msg}", *args, **kwargs)

    def debug(self, msg, *args, **kwargs):
        self.logger.debug(f"[scheduler:{self.node_id}] [pid:{os.getpid()}] - {msg}", *args, **kwargs)
...
class SchedulerConfig:
    ...
    def load_and_validate_config(self):
        ...
        print(f"✅ Scheduler configuration loaded: {self.config_file}")
...
class FirewallScheduler:
    """Scheduler de Firewall - Decision Engine Principal"""
    ...
    def __init__(self, config: SchedulerConfig, firewall_rules_file: str):
        ...
        self.logger.info(f"🚀 Firewall Scheduler initialized: {self.config.node_id}")
    ...
    def start(self):
        ...
        self.running = True
        self.logger.info(f"🚀 Starting Firewall Scheduler {self.config.version}...")
        ...
        self.logger.info("🎯 Firewall Scheduler started successfully")
    ...
    def _log_network_configuration(self):
        ...
        self.logger.info("🌐 Scheduler Network Configuration:")
        self.logger.info("=" * 60)

        # ML Events Input
        self.logger.info(f"📡 ML Events Input (from ml_detector V3.1.2):")
        ...
        self.logger.info(f"🔥 Firewall Commands Output (to firewall_agent):")
        ...
        self.logger.info(f"📥 Firewall Responses Input (from firewall_agent):")
        ...
        # ZMQ Context
        self.logger.info(f"⚙️ ZMQ Context (Conservative config):")
    ...
    def _ml_events_receiver(self, worker_id: int):
        """Recibir eventos del ML Detector V3.1.2 tricapa"""
        self.logger.info(f"📡 ML Events Receiver {worker_id} started for V3.1.2 tricapa")

        while self.running:
            try:
                if self.ml_events_socket:
                    try:
                        # Recibir mensaje (binario protobuf o JSON)
                        message_bytes = self.ml_events_socket.recv(zmq.NOBLOCK)
                        self.stats.events_received += 1

                        self.logger.debug(
                            f"📨 Worker {worker_id} - Received {len(message_bytes)} bytes from ML Detector V3.1.2")
    ...
    def _convert_v3_protobuf_to_dict(self, event, worker_id: int) -> Dict:
        ...
        return {
            ...
            'parsing_method': 'protobuf_v3_scheduler',
            'worker_id': worker_id,
    ...
    def _create_fallback_event(self, message_bytes: bytes, worker_id: int) -> Dict:
        ...
        return {
            ...
            'parsing_method': 'fallback_creation',
            'worker_id': worker_id,
    ...
    def _firewall_command_sender(self, worker_id: int):
        """Sender de comandos de firewall - Thread separado"""
        self.logger.info(f"🔥 Firewall Command Sender {worker_id} started")
        ...
        while self.running:
            try:
                ...
                # Enviar comando al firewall agent
                try:
                    ...
                    self.logger.info(
                        f"📤 Worker {worker_id} - Command sent: {decision.command_id} -> {decision.recommended_action}")
    ...
    def _firewall_responses_receiver(self, worker_id: int):
        ...
        self.logger.info(f"📥 Firewall Responses Receiver {worker_id} started")
                ...
                if self.firewall_responses_socket:
                    try:
                        ...
                        if response_data:
                            ...
                            self.logger.info(f"📥 Worker {worker_id} - Response: {command_id} -> Success: {success}")
    ...
    def _parse_firewall_response(self, response_bytes: bytes, worker_id: int) -> Optional[Dict]:
        """Parser para respuestas del firewall agent"""
        try:
            ...
            try:
                ...
                response_data['parsing_method'] = 'json'
                return response_data
            except Exception:
                pass

            self.logger.warning(f"⚠️ Worker {worker_id} - Could not parse firewall response")
            return None
    ...
    def stop(self):
        ...
        self.logger.info("🛑 Stopping Firewall Scheduler...")
        self.running = False
        ...
        self.logger.info("✅ Firewall Scheduler stopped successfully")
...
def main():
    ...
    if len(sys.argv) != 3:
        print("\n❌ Usage:")
        print("python scheduler-firewall.py <scheduler_config.json> <firewall_rules.json>")
        print("\n📋 File descriptions:")
        print("   • scheduler_config.json: ZMQ, network, processing configuration")
    ...
    try:
        ...
        # Crear scheduler
        print(f"\n🔥 Creating firewall scheduler with JSON rules...")
        scheduler = FirewallScheduler(config, firewall_rules_file)
        ...
        print("📋 Required sections: component, network, zmq, processing, monitoring, logging")
        sys.exit(1)
    except FirewallSchedulerRulesError as e:
        print(f"\n💥 FIREWALL RULES ERROR:")
        print(f"❌ {e}")
        print(f"🔧 Check file: {firewall_rules_file}")
        print("📋 Required sections: firewall_rules.rules, firewall_rules.manual_actions")
        sys.exit(1)
    except json.JSONDecodeError as e:
        print(f"\n💥 JSON FORMAT ERROR:")
        print(f"❌ {e}")
