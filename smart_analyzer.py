...
class SmartAnalyzer:
    def __init__(self, project_root="."):
        ...
        self.ignore_dirs = {
            ...
            ".git",
            "node_modules",
            "upgraded_happiness_venv",
            "upgraded-happiness-bitdefender/venv",
            ".pytest_cache",
            "tests_consolidated",
        }
        ...
        for file_path in self.project_root.rglob("*.py"):
            ...
            if re.match(r"fix_.*\.py$", file_path.name):
                results["fix_scripts"].append(
                    {"path": rel_path, "size": file_size, "name": file_path.name}
                )
            elif ".backup" in file_path.name or ".quick" in file_path.name:
                results["backup_files"].append({"path": rel_path, "size": file_size})
    ...
    def print_concise_report(self, results):
        ...
        print(
            f"   {len(results['backup_files'])} archivos ({round(total_backup_size/1024/1024, 1)} MB)"
        )
        ...
        for essential in results["essential_files"]:
            ...
            print(
                f"   ✅ {Path(essential['path']).name} ({size_kb}KB, {essential['lines']} líneas)"
            )
        ...
        backup_savings = round(
            sum(f["size"] for f in results["backup_files"]) / 1024 / 1024, 1
        )
        ...
        print(
            f"   3. 📊 Enfocar en {len(results['essential_files'])} archivos esenciales"
        )
...
def main():
    ...
    if recommendations["backup_savings_mb"] > 5:
        print(
            f"   Eliminar backups para liberar {recommendations['backup_savings_mb']} MB"
        )
    elif recommendations["fix_scripts_to_consolidate"] > 10:
        print(
            f"   Consolidar {recommendations['fix_scripts_to_consolidate']} scripts fix_*"
        )
    else:
