...
class RealBitDefenderCollector:
    ...
    def _discover_log_files(self):
                ...
                try:
                    for file_path in path.rglob("*"):
                        if (file_path.is_file() and 
                            any(file_path.name.endswith(ext) for ext in log_extensions) and
                            any(keyword in file_path.name.lower() for keyword in ['bitdefender', 'bd', 'antivirus', 'security'])):
        ...
        if not self.log_files:
            self.logger.warning("⚠️ No se encontraron logs de BitDefender. Usando datos de procesos solamente.")
        ...
        while self.running:
            try:
                ...
                for line in result.stdout.splitlines():
                    ...
                    if any(keyword in line.lower() for keyword in ['bitdefender', 'bdl', 'bd']):
                        parts = line.split()
                        if len(parts) >= 11:
                            ...
                            if any(bd_proc in process_name for bd_proc in ['bitdefender', 'BDL', 'bd']):
                                bd_processes.append({
    ...
    async def _process_log_line(self, line: str, source_file: str):
        ...
        if any(keyword in line_lower for keyword in ['virus', 'malware', 'threat', 'infected', 'trojan']):
            event_type = 'malware_detected'
            severity = 'high'
            
            # Extraer nombre de amenaza
            threat_match = re.search(r'(trojan|virus|malware|threat)[:\s]+([^\s,]+)', line_lower)
            if threat_match:
        ...
        elif any(keyword in line_lower for keyword in ['quarantine', 'blocked', 'deleted']):
            event_type = 'threat_blocked'
            severity = 'medium'
        
        # Detección de conexiones sospechosas
        elif any(keyword in line_lower for keyword in ['connection', 'network', 'suspicious']):
            event_type = 'suspicious_connection'
            severity = 'medium'
        
        # Actualizaciones
        elif any(keyword in line_lower for keyword in ['update', 'signature', 'definition']):
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    try:
                        ...
                        for line in result.stdout.splitlines():
                            if any(keyword in line.lower() for keyword in ['bitdefender', 'bdl']):
    ...
    async def _process_syslog_entry(self, entry: Dict):
        ...
        try:
            ...
            if message and any(keyword in message.lower() for keyword in ['bitdefender', 'threat', 'virus']):
                event = RealBitDefenderEvent(
                    timestamp=timestamp,
            ...
            try:
                ...
                stats_event = RealBitDefenderEvent(
                    ...
                    raw_log=json.dumps({
                        ...
                        'monitored_processes': len(self.config['bitdefender']['processes']),
                        'status': 'active'
                    })
                )
